...
// chunk_width = chunk_width * getsScale();
// chunk_height = chunk_height * getsScale();
  ...
  int source_y = 0;
  // while  (source_y+chunk_height < (getsScale()*getFrame()->getHeight() ) ){
  //   int source_x = 0;
  //   while ( source_x+chunk_width < (getsScale()*getFrame()->getWidth() ) ){
  while  (source_y+chunk_height < (getFrame()->getHeight() ) ){
    ...
    while ( source_x+chunk_width < (getFrame()->getWidth() ) ){
      ...
      Chunk* chunk = new Chunk(
                Vector2f((getX()+source_x*getsScale()),   // x coord of destination 
                         (getY()+source_y*getsScale())),  // y coord of destination
                Vector2f(sx, sy),
                getName(),
                frame,getvScale(),getsScale());
